	...
	public List<?> listRelatorioGestaoMensal(RelatorioGestaoMensalDTO relatorioGestaoMensal) throws Exception {
		...
		try{
			result =  HibernateUtility.getSession().createSQLQuery("select "
						...
						+ " (select count(*) from usuario u inner join financeiro f on f.usuario_id = u.id where f.mes = financeiro.mes and (excluido=0 or excluido is null) group by mes) as total_usuario,"
						+ " (select count(*) from usuario u inner join financeiro f on f.usuario_id = u.id where f.mes = financeiro.mes and (excluido=0 or excluido is null) and ativo_inativo = 1 group by mes) ativo,"
						+ " (select count(*) from usuario u inner join financeiro f on f.usuario_id = u.id where f.mes = financeiro.mes and (excluido=0 or excluido is null) and ativo_inativo = 0 group by mes) inativo,"
						+ " (select count(*) from financeiro f inner join usuario u on f.usuario_id = u.id where f.mes = financeiro.mes and (excluido=0 or excluido is null) and situacao = 1 ) sem_pendencia,"
						+ " (select count(*) from financeiro f inner join usuario u on f.usuario_id = u.id where f.mes = financeiro.mes and (excluido=0 or excluido is null) and situacao = 0) com_pendencia,"
						+ " (select sum(valor_com_desconto) from financeiro f inner join usuario u on f.usuario_id = u.id where f.mes = financeiro.mes and (excluido=0 or excluido is null)) total_arrecadado"
						+ " ,mes,ano"
